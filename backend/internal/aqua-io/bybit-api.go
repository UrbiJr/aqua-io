...
type OrderData struct {
	...
	Category    utils2.ByBitProductType `json:"category"`
	Symbol      string                  `json:"symbol"`
	Side        utils2.OrderSide        `json:"side"`
	OrderType   utils2.OrderType        `json:"orderType"`
...
func (app *Config) createOrder(p *user2.Profile, orderData OrderData) (*user2.OpenedPosition, error) {
	var url string
			...
			if parsed["retMsg"].(string) == "OK" {
				...
				for key, _ = range result {
					if key == "orderId" {
						...
						return &user2.OpenedPosition{OrderID: result[key].(string), Symbol: orderData.Symbol, ProfileID: p.ID}, nil
					}
				}
			} else {
...
func (app *Config) cancelOrder(p *user2.Profile, category, orderID, symbol string) error {
	var url string
...
func (app *Config) openShortPosition(p *user2.Profile, orderData OrderData) (*user2.OpenedPosition, error) {

	if utils2.Contains(p.BlacklistCoins, orderData.Symbol) {
		return nil, fmt.Errorf("cannot open short position: symbol %s is in user blacklisted coins", orderData.Symbol)
	}

	orderData.Category = utils2.BYBIT_PRODUCT_LINEAR
	orderData.Side = utils2.ORDER_SELL
...
func (app *Config) openLongPosition(p *user2.Profile, orderData OrderData) (*user2.OpenedPosition, error) {
	...
	orderData.Category = utils2.BYBIT_PRODUCT_LINEAR
	orderData.Side = utils2.ORDER_BUY
	orderData.PositionIdx = 0
	orderData.ReduceOnly = false
...
func (app *Config) closeShortPosition(p *user2.Profile, orderData OrderData) (*user2.OpenedPosition, error) {
...
func (app *Config) closeLongPosition(p *user2.Profile, orderData OrderData) (*user2.OpenedPosition, error) {
...
func (app *Config) getBybitTransactions(p user2.Profile) []user2.Transaction {
	var url string
	...
	var parsed map[string]any
	var transactions []user2.Transaction
			...
			for key, _ := range result {
				if key == "list" && parsed["list"] != nil {
					...
					for _, x := range list {
						...
						transactions = append(transactions, user2.Transaction{
							ProfileID:       p.ID,
...
func (app *Config) fetchOrderHistory(category, orderFilter string, p user2.Profile) ([]user2.Order, error) {
	var url string
				...
				if key == "list" && result["list"] != nil {
					...
					for _, x := range list {
						...
						case map[string]any:
							...
							orders = append(orders, user2.Order{
								ProfileID:        p.ID,
...
func (app *Config) fetchOpenOrders(category string, p user2.Profile) []user2.Order {
	var url string
...
func (app *Config) getPositionInfo(category, symbol, orderID string, p user2.Profile) []user2.PositionInfo {
	var url string
				...
				if key == "list" && result["list"] != nil {
					...
					for _, x := range list {
						...
						case map[string]any:
							...
							positionInfoArr = append(positionInfoArr, user2.PositionInfo{
								OrderID:        orderID,
